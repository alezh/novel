...
import (
	...
	"github.com/alezh/novel/system/data/history"
	"time"
	"sync/atomic"
	"sort"
)
...
func (self *Matrix) Push(req *request.Request) {
	...
	if mis.checkStatus(config.STOP) {
		return
	}
	...
	// 暂停状态时等待，降低请求积存量
	waited := false
	for mis.checkStatus(config.PAUSE) {
		waited = true
		time.Sleep(time.Second)
	}
	if waited && mis.checkStatus(config.STOP) {
		return
	}

	// 资源使用过多时等待，降低请求积存量
	waited = false
	for atomic.LoadInt32(&self.resCount) > mis.avgRes() {
		waited = true
...
func (self *Matrix) Pull() (req *request.Request) {
	...
	for i := len(self.reqs) - 1; i >= 0; i-- {
		...
		if len(self.reqs[idx]) > 0 {
			...
			if mis.useProxy {
				//req.SetProxy(mis.proxy.GetOne(req.GetUrl()))
			} else {
				req.SetProxy("")
			}
			return
...
func (self *Matrix) TryFlushSuccess() {
	if config.Task.Mode != config.SERVER && config.Task.SuccessInherit {
		self.history.FlushSuccess(config.Task.OutType)
	}
}

// 非服务器模式下保存历史失败记录
func (self *Matrix) TryFlushFailure() {
	if config.Task.Mode != config.SERVER && config.Task.FailureInherit {
		self.history.FlushFailure(config.Task.OutType)
	}
}
